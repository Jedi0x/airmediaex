...
$(function() {
    ...
    $("body").on('change', 'input[name="shipping_active"]', function() {
       if($(this).prop('checked')) {
            $(".shipping_box").removeClass('hide');
        } else {
            $(".shipping_box").addClass('hide');
        }
    });
    ...
    $("body").on('change', 'input[name="payment_terms_active"]', function() {
       if($(this).prop('checked')) {
            $(".payment_terms_box").removeClass('hide');
        } else {
            $(".payment_terms_box").addClass('hide');
        }
    });
    ...
    $("body").on('change', 'input[name="labour_terms_active"]', function() {
       if($(this).prop('checked')) {
            $(".labour_terms_box").removeClass('hide');
        } else {
            $(".labour_terms_box").addClass('hide');
        }
    });
    ...
    $("body").on('change', 'input[name="term_and_conditions_active"]', function() {
       if($(this).prop('checked')) {
            $(".term_and_conditions_box").removeClass('hide');
        } else {
            $(".term_and_conditions_box").addClass('hide');
        }
    });
    ...
    $('body').on('click', '.discount-group-total-type', function(e) {
        ...
        $('#discount-group-total-type-dropdown'+item_id).find('.discount-group-total-type').removeClass('selected');
        $(this).addClass('selected');
        $('#discount-total-'+item_id+' .discount-group-total-type-selected').html($(this).text());
        if ($(this).hasClass('discount-type-percent')) {
            $('#discount-total-'+item_id+' .input-discount-group-fixed').addClass('hide').val(0);
            $('#discount-total-'+item_id+' .input-discount-group-percent').removeClass('hide');
            $('#discount-total-'+item_id+' .input-discount-group-percent').addClass('shOw');
            $('#discount-total-'+item_id+' .input-discount-group-fixed').removeClass('shOw');
        } else {
            $('#discount-total-'+item_id+' .input-discount-group-fixed').removeClass('hide');
            $('#discount-total-'+item_id+' .input-discount-group-fixed').addClass('shOw');
            $('#discount-total-'+item_id+' .input-discount-group-percent').addClass('hide').val(0);
            $('#discount-total-'+item_id+' .input-discount-group-percent').removeClass('shOw');
            $('#discount_percent-error',this).remove();
        }
        // calculate_total();
        // calculate_total_group(0,$(this).attr('data-group-id'));
    });
...
function init_items_sortable(preview_table) {
    ...
    $("tbody.connectedSortable")
        ...
        receive: function(event, ui) {
            var addedTo = $(this).closest("table.invoice-items-table"),
            removedFrom = $("table.invoice-items-table").not(addedTo);
            $(ui.item).find("input.item_group_id").val(addedTo.attr("data-group-id")) ;
            calculate_total_group(0,addedTo.attr("data-group-id"));
            calculate_total_group(0,removedFrom.attr("data-group-id"));
        }
...
function reorder_items() {
    // bitsclan
   var rows = $('.items tbody tr.item');
    //var rows = $('.group_items tr.item');
    var i = 1;
    $.each(rows, function() {
...
function add_group_item_to_preview(id) {
    requestGetJSON('invoice_items/get_group_by_id/' + id).done(function(response) {
        clear_item_preview_values();
        $(".group_items").append(response.template);
        $('select.taxX'+response.group.id+'').selectpicker();
        $.each(response.items, function(index, element) { 
            var taxSelectedArray = [];
            if (element.taxname && element.taxrate) {
                taxSelectedArray.push(element.taxname + '|' + element.taxrate);
            }
            if (element.taxname_2 && element.taxrate_2) {
                taxSelectedArray.push(element.taxname_2 + '|' + element.taxrate_2);
            }

            $('.item'+element.itemid+' select.tax'+element.itemid+'').selectpicker('val', taxSelectedArray);
            $('.item'+element.itemid+' input[name="unit"]').val(element.unit);
            
        });
       
        calculate_total_group(0,response.group.id);
...
function get_group_item_preview_values(id) {
    var response = {};
    response.description = $('.item'+id+' .main textarea[name="description"]').val();
    response.long_description = $('.item'+id+' .main textarea[name="long_description"]').val();
    response.qty = $('.item'+id+' .main input[name="quantity"]').val();
    response.taxname = $('.item'+id+' .main select.taxX'+id).selectpicker('val');
    response.rate = $('.item'+id+' .main input[name="rate"]').val();
    response.unit = $('.item'+id+' .main input[name="unit"]').val();

    if($('.item'+id+' .main input[name="discount_group_percent"]').hasClass('hide')){
        response.discount = $('.item'+id+' .main input[name="discount_group_total"]').val();
        response.discount_type = 'fix';
    }else{
        response.discount = $('.item'+id+' .main input[name="discount_group_percent"]').val();
...
function add_group_item_to_table(data, itemid, merge_invoice, bill_expense,group_id) {

    // If not custom data passed get from the preview
    data = typeof(data) == 'undefined' || data == 'undefined' ? get_group_item_preview_values(group_id) : data;
    if (data.description === "" && data.long_description === "" && data.rate === "") { return; }

    var table_row = '';

    var item_key = rand_num();
    ...
    if(data.discount_type == 'fix'){
        ...
        var percentage = data.discount;
        discount = (percentage / 100) * (data.rate * data.qty);
    }

    var amount = data.rate * data.qty - discount;
    ...
    get_taxes_dropdown_template(tax_name, data.taxname).done(function(tax_dropdown) {
        ...
        if(data.discount_type == 'fix'){
            ...
            percentage_select = "";
            fix_class = 'shOw';
            discount_text = "Fixed Amount";
            percentage_class = 'hide';
            fix_discount = data.discount;
        }else{
            fix_select = "";
            percentage_select = "selected";
            fix_class = 'hide';
            percentage_class = 'shOw';
            discount_text = "%";
            percentage_discount = data.discount;
        }


        table_row += '<input type="hidden" name="newitems[' + item_key + '][group_id]" value="' + group_id + '">';
        ...
        table_row += '<td class="discount"> <div class="input-group" id="discount-total-' + item_key + '">';

        table_row += '<input type="number" data-discount  class="'+ percentage_class +'  form-control pull-left input-discount-group-percent" min="0" max="100" name="newitems[' + item_key + '][discount_group_percent]" data-group-id = "' + group_id + '" value="'+percentage_discount+'">';

        table_row += '<input type="number" data-discount data-toggle="tooltip" data-title="The number in the input field is not formatted while edit/add item and should remain not formatted do not try to format it manually in here." value="'+fix_discount+'" class="form-control pull-left input-discount-group-fixed '+ fix_class +' " min="0" name="newitems[' + item_key + '][discount_group_total]" data-group-id = "' + group_id + '">';
        
        table_row += '<div class="input-group-addon"><div class="dropdown">';

        table_row += '<a class="dropdown-toggle" href="#" id="dropdown_menu_tax_total_type_1" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">';

        table_row += '<span class="discount-group-total-type-selected">'+discount_text+'</span><span class="caret"></span>';

        table_row += '</a>';

        table_row += '<ul class="dropdown-menu discount-group-total-type-dropdown" id="discount-group-total-type-dropdown' + item_key + '" aria-labelledby="dropdown_menu_tax_total_type">';

        table_row += '<li><a href="#" class="discount-group-total-type discount-type-percent '+ percentage_select +'" data-group-id = "' + group_id + '" data-item-id = "' + item_key + '">%</a></li>';

        table_row += '<li><a href="#" class="discount-group-total-type discount-type-fixed '+ fix_select +'" data-group-id = "'+ group_id +'" data-item-id = "' + item_key + '">Fixed Amount</a></li>';
        ...
        table_row += '<td class="amount-group" align="right">' + format_money(amount, true) + '</td>';

        table_row += '<td><a href="#" class="btn btn-danger pull-left" onclick="delete_item(this,' + itemid + ',' + group_id + '); return false;"><i class="fa fa-trash"></i></a></td>';

        table_row += '</tr>';

        $('table.table-item-group'+group_id+' tbody').append(table_row);
        ...
        setTimeout(function() {
            calculate_total_group(0,group_id);
        }, 25);
...
function delete_item_group(id) {

    setTimeout(function() {
        $('.item-group-'+id).remove();
        
    }, 50);
    setTimeout(function() {
        calculate_total_group(0,id);
    }, 50);

    $('.item-group-'+id+' .itemid').each(function() {
        if ($('input[name="isedit"]').length > 0) {
            $('#removed-items').append(hidden_input('removed_items[]', $(this).val()));
        }
    });
...
$( ".group_items" ).sortable( {
    ...
    $(this).children().each(function(index) {
        $(this).find('td').last().append(hidden_input('group_order['+$(this).attr('data-group-id')+']', index + 1))
    });
  }
});
...
function calculate_total_group(itemid='',groupid='') {
    ...
    var calculated_tax,
        ...
        $.each(rows, function() {
            ...
            total_amount_ = $(this).find('td.rate input').val() * quantity;
            discount = $(this).find('td.discount input.shOw').val();
            ...
            if(!isNaN(discount)){
                if($(this).find('td.discount input.shOw').hasClass('input-discount-group-fixed')){
                    ...
                    discounted_value = (discount / 100) * total_amount_ ;
                }
                _amount = accounting.toFixed($(this).find('td.rate input').val() * quantity - parseFloat(discounted_value), app.options.decimal_places);
                _amount = parseFloat(_amount);
                subtotal+=_amount;
                row.find('td.amount-group').html(format_money(_amount, true));
            }
        ...
        if(groupid != ""){
            ...
            $('.item'+groupid+' tr').each(function() {
                ...
                if(!isNaN(discount)){
                    ...
                    _amount = accounting.toFixed($(this).find('td.rate input').val() * quantity - parseFloat(discounted_value), app.options.decimal_places);
                    _amount = parseFloat(_amount);
                    $(this).find('td.amount-group').html(format_money(_amount, true));
                    group_sub_total += _amount;
                }
            ...
            // console.log(group_sub_total);

            $('.total'+groupid).attr('data-amount',group_sub_total);
            $('.total'+groupid).html(format_money(group_sub_total)); 
...
//bitsclan solution
function delete_item(row, itemid , group_id='') {
    $(row).parents('tr').addClass('animated fadeOut', function() {
        var remove_amount = $(this).find('td.rate input').val();
        setTimeout(function() {
             $(row).parents('tr').remove();
            if(group_id == ""){
                calculate_total();
            }else{
                calculate_total_group(itemid,group_id);
            }
        }, 50);
    });
    ...
    $( ".group_items-drag" ).sortable( {
        update: function( event, ui ) {
            ...
            $.ajax({
                    ...
                    data:{invoice_id:invoice_id,group_id:group_id,order:order},
                    dataType:'json',
