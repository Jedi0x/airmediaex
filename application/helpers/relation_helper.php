...
function get_relation_data($type, $rel_id = '')
{
    ...
    elseif ($type == 'opportunity' || $type == 'opportunity') { 
        
            $where_opportunity = '';
        if ($q) {
            $where_opportunity .= '(project_name LIKE "%' . $CI->db->escape_like_str($q) . '%")';
        }
        $CI->load->model(OPPORTUNITIES_MODULE . '/Opportunity_model','opportunity');
        $data = $CI->opportunity->get($rel_id,$where_opportunity);
...
function get_relation_values($relation, $type)
{
    ...
    elseif ($type == 'opportunity' || $type == 'opportunity') { 
        ...
        $link = admin_url('opportunities/opportunity/' . $id);
    }
    // Bitsclan Solutions End Code
     elseif ($type == 'contact' || $type == 'contacts') {
        if (is_array($relation)) {
