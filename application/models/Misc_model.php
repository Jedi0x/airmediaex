...
class Misc_model extends App_Model
{
    ...
    public function get_taxes_dropdown_template($name, $taxname, $type = '', $item_id = '', $is_edit = false, $manual = false,$class_append = '')
    {
        ...
        $select = '<select class="selectpicker display-block tax'.$class_append.'" data-width="100%" name="' . $name . '" multiple data-none-selected-text="' . _l('no_tax') . '">';
    ...
    public function add_attachment_to_database($rel_id, $rel_type, $attachment, $external = false)
    {
        ...
        hooks()->do_action('after_file_added_opportunity_activity', $data);
        ...
        if ($data['rel_type'] == 'opportunity' && isset($data['opportunity_id'])) {
            
            $this->db->insert(db_prefix() . 'shared_opportunity_files', [
                'file_id'    => $insert_id,
                'contact_id' => $data['opportunity_id'],
            ]);
        }
        // Bitsclan Solutions End Code
