...
$statuses  = $ci->estimates_model->get_statuses();
$statusIds = [];
...
if (isset($opportunity) && $opportunity != '') {
    array_push($where, 'AND ' . db_prefix() . 'estimates.opportunity=' . $this->ci->db->escape_str($opportunity));
}else{
    array_push($where, 'AND ' . db_prefix() . 'estimates.opportunity=0');
}
