...
class Download extends App_Controller
{
    ...
    public function file($folder_indicator, $attachmentid = '')
    {
        ...
        elseif ($folder_indicator == 'opportunity') {
            $this->db->where('attachment_key', $attachmentid);
            $attachment = $this->db->get(db_prefix().'files')->row();
            if (!$attachment) {
                show_404();
            }
            if (has_permission('opportunities', '', 'view') || is_customer_admin($attachment->rel_id) || is_client_logged_in()) {
                $path = get_upload_path_by_type('opportunity') . $attachment->rel_id . '/' . $attachment->file_name;
            }
        }
