...
$(function() {
    ...
    if (contact_id) {
        contact(opportunity_id, contact_id);
    }
    ...
    $('.opportunity-form-submiter').on('click', function() {
        var form = $('.opportunity-form');
        if (form.valid()) {
   ...
   $(document).on('click', '.edit_call_log', function(e){
    var id = $(this).attr('data-id');
    $.ajax({
        type: "POST",
        url: admin_url+'opportunities/edit_call_log',
        data:{id:id},
        success: function(res){
            $(".show_body").html(res);
            $("#call_log_edit").modal('show');
            var dateToday = new Date(); 
    ...
    initDataTable('.table-tickets-single', admin_url + 'tickets/index/false/' + opportunity_id, undefined, undefined, 'undefined', [$('table thead .ticket_created_column').index(), 'desc']);

    /* Customer profile contracts table */
    initDataTable('.table-contracts-single-client', admin_url + 'contracts/table/' + opportunity_id, undefined,undefined, 'undefined', [6, 'desc']);
    ...
    <?php } ?>
    _table_api = initDataTable('.table-contacts', admin_url + 'clients/contacts/' + opportunity_id, contactsNotSortable, contactsNotSortable);
    if(_table_api) {
          <?php if(is_gdpr() && get_option('gdpr_enable_consent_for_contacts') == '1'){ ?>
    ...
    initDataTable('.table-invoices-single-client',
        admin_url + 'invoices/table/' + opportunity_id,
        'undefined',
        'undefined',
        'undefined', [
            [3, 'desc'],
   ...
   initDataTable('.table-credit-notes', admin_url+'credit_notes/table/'+opportunity_id, ['undefined'], ['undefined'], undefined, [0, 'desc']);

    /* Customer profile Estimates table */
    initDataTable('.table-estimates-single-client',
        admin_url + 'opportunities/estimate_table_opportunity/' + opportunity_id,
        'undefined',
        'undefined',
        'undefined', [
            [3, 'desc'],
    ...
    initDataTable('.table-payments-single-client',
        admin_url + 'payments/table/' + opportunity_id, undefined, undefined,
        'undefined', [0, 'desc']);

    /* Customer profile reminders table */
    initDataTable('.table-reminders', admin_url + 'misc/get_reminders/' + opportunity_id + '/' + 'opportunity', undefined, undefined, undefined, [1, 'asc']);

    /* Customer profile expenses table */
    initDataTable('.table-expenses-single-client',
        admin_url + 'opportunities/expenses_table_opportunity/' + opportunity_id,
        'undefined',
        'undefined',
        'undefined', [5, 'desc']);
    ...
    initDataTable('.table-proposals-client-profile',
        admin_url + 'proposals/proposal_relations/' + opportunity_id + '/opportunity',
        'undefined',
        'undefined',
        'undefined', [6, 'desc']);

    /* Custome profile projects table */
    initDataTable('.table-projects-single-client', admin_url + 'projects/table/' + opportunity_id, undefined, undefined, 'undefined', <?php echo hooks()->apply_filters('projects_table_default_order', json_encode(array(5,'asc'))); ?>);

    /* opportunity task  table */
     var url = admin_url + 'tasks/init_relation_tasks/' + opportunity_id + '/' + 'opportunity';
    initDataTable('.table-rel-tasks', url , undefined, undefined, 'undefined', <?php echo hooks()->apply_filters('projects_table_default_order', json_encode(array(5,'asc'))); ?>);
    ...
    if(typeof(opportunity_id) == 'undefined'){
...
function do_share_file_contacts(edit_contacts, file_id) {
    var contacts_shared_ids = $('select[name="share_opportunity_id[]"]');
    if (typeof(edit_contacts) == 'undefined' && typeof(file_id) == 'undefined') {
        var opportunity_shared_ids_selected = $('select[name="share_opportunity_id[]"]').val();
    } else {
    ...
    $.post(admin_url + 'opportunities/update_file_share_visibility', {
        file_id: file_id,
        share_opportunities_id: opportunity_shared_ids_selected,
...
function fetch_lat_long_from_google_cprofile() {
 ...
 function set_status(status) {
    console.log(status);
          if(status == 5){
        $(".closed_status").removeClass('hide');
    }else{
        $(".closed_status").addClass('hide');
    }
    }


    function edit_note_opportunity(id,opportunity) {
    var description = $("body").find('[data-note-edit-textarea="' + id + '"] textarea').val();
    if (description !== '') {
        $.post(admin_url + 'opportunities/edit_note/' + id, {
            description: description, opportunity: opportunity
        }).done(function(response) {
            response = JSON.parse(response);
...
$(document).on('change', '#account', function(e){
    ...
    $.ajax({
        type: "POST",
        url: admin_url+'opportunities/get_contacts',
        data:{id:id},
        success: function(res){
