...
<a href="<?php echo admin_url('proposals/proposal?rel_type=opportunity&rel_id='.$opportunity->id); ?>" class="btn btn-info mbot25"><?php echo _l('new_proposal'); ?></a>
<?php } ?>
<?php if(total_rows(db_prefix().'proposals',array('rel_type'=>'opportunity','rel_id'=>$opportunity->id))> 0 && (has_permission('proposals','','create') || has_permission('proposals','','edit'))){ ?>
<a href="#" class="btn btn-info mbot25" data-toggle="modal" data-target="#sync_data_proposal_data"><?php echo _l('sync_data'); ?></a>
<?php $this->load->view('admin/proposals/sync_data',array('related'=>$opportunity,'rel_id'=>$opportunity->id,'rel_type'=>'opportunity')); ?>
<?php } ?>
<?php
$table_data = array(
 _l('proposal') . ' #',
