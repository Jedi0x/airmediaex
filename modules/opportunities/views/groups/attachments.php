...
<div class="modal fade" id="customer_file_share_file_with" data-total-contacts="<?php echo count($opportunities); ?>" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        ...
        <div class="modal-body">
            ...
            <?php echo render_select('share_opportunity_id[]',$opportunities,array('id',array('project_name')),'opportunities',array(get_primary_contact_user_id($opportunity->id)),array('multiple'=>true,'data-actions-box'=>true),array(),'','',false); ?>
        </div>
...
<br />
<small class="text-info"><?php echo _l('opportunity_files_info_message'); ?></small>
</h4>
<hr />
<?php if(isset($opportunity)){ ?>
    <?php echo form_open_multipart(admin_url('opportunities/upload_attachment/'.$opportunity->id),array('class'=>'dropzone','id'=>'client-attachments-upload')); ?>
                ...
                <tbody>
                    <?php foreach($attachments as $type => $attachment){
                        ...
                        if($type == 'invoice' || $type == 'proposal' || $type == 'estimate' || $type == 'credit_note'){
                            ...
                            $url = site_url() .'download/file/opportunity/';
                            $download_indicator = 'attachment_key';
                        }
                        ?>
                        <?php foreach($attachment as $_att){
                         ...
                         <td>
                            <div class="onoffswitch"<?php if($type != 'opportunity'){?> data-toggle="tooltip" data-title="<?php echo _l('customer_attachments_show_notice'); ?>" <?php } ?>>
                                <input type="checkbox" <?php if($type != 'opportunity'){echo 'disabled';} ?> id="<?php echo $_att['id']; ?>" data-id="<?php echo $_att['id']; ?>" class="onoffswitch-checkbox customer_file" data-switch-url="<?php echo admin_url(); ?>misc/toggle_file_visibility" <?php if(isset($_att['visible_to_customer']) && $_att['visible_to_customer'] == 1){echo 'checked';} ?>>
                                <label class="onoffswitch-label" for="<?php echo $_att['id']; ?>"></label>
                            </div>
                            <?php if($type == 'opportunity' && $_att['visible_to_customer'] == 1){
                                ...
                                echo '<span class="text-warning'.(empty($file_visibility_message) || total_rows(db_prefix().'contacts',array('userid'=>$opportunity->id)) == 0 ? ' hide': '').'">'.$file_visibility_message.'</span>';
                        ...
                        <td>
                            ...
                            <?php if($type == 'opportunity'){ ?>
                                <a href="<?php echo admin_url('opportunities/delete_attachment/'.$_att['rel_id'].'/'.$_att['id']); ?>"  class="btn btn-danger btn-icon _delete"><i class="fa fa-remove"></i></a>
                            <?php } ?>
                        </td>
                    <?php } ?>
                </tr>
...
<?php
$this->load->view('modals/send_file_modal');
} ?>
